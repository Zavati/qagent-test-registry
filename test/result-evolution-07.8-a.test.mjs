...
test('07.8-A creates one immutable derived version and replays proposal idempotently',async()=>{const db=new SQLiteD1();applyCurrentMigrations(db);
  try{const base=appendPayload({generationRequestId:'tdg_evolution_base_0001'});base.specification.scenarios[0].spec.assertions=[{type:'STATUS',expectedStatusCodes:[200]},{type:'CONTENT_TYPE',expected:['application/json']}];const created=await handleRequest(new Request('https://r/v1/test-registry/test-designs/versions',{method:'POST',headers,body:JSON.stringify(base)}),env(db));assert.equal(created.status,201);const first=(await created.json()).data.testDesign;
